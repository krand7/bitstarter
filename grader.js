...
//	}
//	else {
//	    console.error("%s was written.", instr);
//	    fs.writeFileSync(instr,result);
//	}
  //  });
//};
...
var cheerioURLFile = function(url_name, checksfile,  callback) {
	...
	console.log('This makes it here');
	var checks=loadChecks(checksfile).sort();
	console.log(checks)
	$ = cheerio.load(result);
	var out={};
...
var checkURL2 = function(url_name, checksfile) {
    var checks=loadChecks(checksfile).sort();
    var out={};
    for (var ii in checks) {
	var present = $(checks[ii]).length>0;
...
if(require.main == module) {
	...
	.option('-u, --url <url_address>', 'Path to checks.json', 'A string', HTMLFILE_DEFAULT)
        .parse(process.argv);
